...
function service_page_callback() {
    ...
    if (!empty($services)) {
        ...
        foreach ($services as $service) {
            ...
            $service_name = get_post_meta($service_id, '_service_name', true);
            $price = get_post_meta($service_id, '_service_price', true);
            $vat = get_post_meta($service_id, '_service_vat', true);
            $start_date = get_post_meta($service_id, '_service_start_date', true);
            $end_date = get_post_meta($service_id, '_service_end_date', true);
